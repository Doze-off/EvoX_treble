...
dependency_install(){
    ...
    if [[ "$OSTYPE" == "linux-gnu" ]]; then
        ...
        sudo apt install git p7zip openjdk-8-jdk curl cpio wget unace unrar zip unzip p7zip-full p7zip-rar sharutils uudeview mpack arj cabextract file-roller aptitude device-tree-compiler liblzma-dev liblz4-tool gawk aria2 -y
...
python_fix_install(){
    id_name(){
        lsb_release -a &> id.ver
        cat id.ver | head -n 2 | grep -o "Ubuntu"
        rm -rf id.ver
    }
    if [[ id_name ]] ; then
        echo -e "\033[33m [DEBUG] Ubuntu Python and Pip install... \033[0m"
        ...
        echo -e "\033[33m [INFO] Python2 && pip2 and Python3 && pip3 install... \033[0m"
        sudo apt install python python3-pip python-pip python3-pip -y
    fi
}
